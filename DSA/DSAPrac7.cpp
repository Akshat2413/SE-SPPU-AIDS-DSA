...
void tree::input() {
    ...
    if (v <= 0 || v > MAX_VERTICES) {
        cout << "Invalid number of branches. Please enter a value between 1 and " << MAX_VERTICES << endl;
        return;
    }
    cout << "Enter the no. of connections: ";
    cin >> e;
    if (e < 0 || e > v * (v - 1) / 2) {
        cout << "Invalid number of connections. Please enter a value between 0 and " << v * (v - 1) / 2 << endl;
        return;
    }
    int l, u, w;
    for (int i = 0; i < e; i++) {
